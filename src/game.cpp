...
Game::Game() : stack_(std::make_unique<Stack>()) {
    ...
    stack_->shuffle();
    std::unique_ptr<Card> endCard =
        CardFactory::createCard("CEO Death", cards.size() + 1);
...
void Game::initialize(const std::vector<UserPlayerInput>& players) {
    std::vector<std::unique_ptr<CharacterCard>> cards =
        stack_->fetchPlayersCards(players.size());
    ...
    for (int i = 0; static_cast<size_t>(i) < cards.size(); ++i) {
        ...
        std::string playerName =
            player->getPlayerName() + " (" + card->getName() + ")";
...
std::unique_ptr<Move> Game::offerMove() {
    ...
    std::unique_ptr<Move> move =
        std::make_unique<Move>(*actor, std::move(card));
...
std::shared_ptr<Player> Game::determineWinner() const {
    ...
    for (std::shared_ptr<Player> player : players_) {
        ...
        if (player->getTrust() == trust &&
            player->getReputation() > winner->getReputation()) {
            winner = player;
            continue;
        }
        if (player->getTrust() == trust &&
            player->getReputation() == winner->getReputation() &&
            player->getMoney() > winner->getMoney()) {
            winner = player;
            continue;
        }
    }
