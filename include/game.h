...
public:
     ...
     * Creates player instances, initializes card deck, and determines turn order
     * 
     ...
     * 
     * @return const std::vector<std::shared_ptr<Player>> Vector of player pointers
     */
...
private:
    ...
    std::queue<std::shared_ptr<Player>> playersQueue_; ///< Current turn order
}; 