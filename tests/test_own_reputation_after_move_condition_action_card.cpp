...
class OwnReputationAfterMoveConditionActionCardTest : public ::testing::Test {
    protected:
        void SetUp() override {
            card = std::make_unique<OwnReputationAfterMoveConditionActionCard>(
                1, "Test card", 10, 5, 50);
...
TEST_F(OwnReputationAfterMoveConditionActionCardTest,
       GetMinReputationReturnsCorrectValue) {
    EXPECT_EQ(card->getMinReputation(), 50);
}

TEST_F(OwnReputationAfterMoveConditionActionCardTest,
       IsPossibleToPlayReturnsTrueWhenReputationWillMeetCondition) {
    player->gainReputation(50);
    EXPECT_TRUE(card->isPossibleToPlay(*player));
}

TEST_F(OwnReputationAfterMoveConditionActionCardTest,
       IsPossibleToPlayReturnsFalseWhenReputationWillNotMeetCondition) {
