...
class TrustBeforeConditionLeverageCardTest : public ::testing::Test {
    protected:
        void SetUp() override {
            card = std::make_unique<TrustBeforeConditionLeverageCard>(
                1, "Test card", -1, -5, -4, 40);
...
TEST_F(TrustBeforeConditionLeverageCardTest,
       IsPossibleToPlayReturnsTrueWhenTrustMeetsCondition) {
    player->gainTrust(25);
