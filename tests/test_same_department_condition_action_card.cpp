...
class SameDepartmentConditionActionCardTest : public ::testing::Test {
    protected:
        void SetUp() override {
            card = std::make_unique<SameDepartmentConditionActionCard>(
                1, "Test card", 10, 5, "development");
...
TEST_F(SameDepartmentConditionActionCardTest,
       GetDepartmentReturnsCorrectValue) {
    EXPECT_EQ(card->getDepartment(), "development");
}

TEST_F(SameDepartmentConditionActionCardTest,
       IsPossibleToPlayReturnsTrueWhenDepartmentMatches) {
    player->setDepartment("development");
    EXPECT_TRUE(card->isPossibleToPlay(*player));
}

TEST_F(SameDepartmentConditionActionCardTest,
       IsPossibleToPlayReturnsFalseWhenDepartmentDoesNotMatch) {
