...
class AllyReputationConditionActionCardTest : public ::testing::Test {
    protected:
        void SetUp() override {
            card = std::make_unique<AllyReputationConditionActionCard>(
                1, "Test Card", 10, 5, 40);
            player = std::make_unique<Player>("test_id", "test_secret");
            character = std::make_unique<CharacterCard>(2, "Ally", 50, 20,
                                                        "development");
        }
...
TEST_F(AllyReputationConditionActionCardTest,
       GetMinAllyReputationReturnsCorrectValue) {
    EXPECT_EQ(card->getMinAllyReputation(), 40);
}

TEST_F(AllyReputationConditionActionCardTest,
       IsPossibleToPlayReturnsTrueWhenAllyMeetsCondition) {
...
TEST_F(AllyReputationConditionActionCardTest,
       IsPossibleToPlayReturnsFalseWhenNoAllyMeetsCondition) {
    player->setDepartment("development");
    auto weakAlly =
        std::make_unique<CharacterCard>(3, "Weak ally", 30, 10, "development");
