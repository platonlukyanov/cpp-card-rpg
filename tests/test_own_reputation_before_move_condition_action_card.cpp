...
TEST_F(OwnReputationBeforeMoveConditionActionCardTest, GetMinReputationReturnsCorrectValue) {
    EXPECT_EQ(card->getMinReputation(), 50);
}

TEST_F(OwnReputationBeforeMoveConditionActionCardTest, IsPossibleToPlayReturnsTrueWhenReputationMeetsCondition) {
    player->gainReputation(60);
    EXPECT_TRUE(card->isPossibleToPlay(*player));
}

TEST_F(OwnReputationBeforeMoveConditionActionCardTest, IsPossibleToPlayReturnsFalseWhenReputationBelowCondition) {
    player->gainReputation(40);
    EXPECT_FALSE(card->isPossibleToPlay(*player));
}

TEST_F(OwnReputationBeforeMoveConditionActionCardTest, ExecuteUpdatesPlayerStats) {
    player->gainReputation(60);
    card->execute(*player);
    
    EXPECT_EQ(player->getReputation(), 61); // 60 + 1 (10% от 10)
